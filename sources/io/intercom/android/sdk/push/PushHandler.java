...
class PushHandler {
    ...
    private boolean hasUserHashIfNeeded(UserIdentity userIdentity, AppConfig appConfig) {
        if (!appConfig.isReceivedFromServer() || (appConfig.isIdentityVerificationEnabled() && (!appConfig.isIdentityVerificationEnabled() || TextUtils.isEmpty(userIdentity.getHmac())))) {
            return false;
        }
        return true;
    }

    private void sendNotification(Bundle bundle, SystemNotificationManager systemNotificationManager, boolean z10, Context context, MetricTracker metricTracker, AppConfig appConfig) {
        if (bundle.isEmpty()) {
            this.twig.e("The push intent contained no data", new Object[0]);
            return;
        }
        Twig twig2 = this.twig;
        twig2.internal(TAG, "Notification Data Json :" + bundle.getString(MetricTracker.Object.MESSAGE));
    ...
    public void handlePush(Bundle bundle, UserIdentity userIdentity, SystemNotificationManager systemNotificationManager, boolean z10, Context context, MetricTracker metricTracker, AppConfig appConfig) {
        if (userIdentity.identityExists()) {
            sendNotification(bundle, systemNotificationManager, z10, context, metricTracker, appConfig);
            return;
        }
