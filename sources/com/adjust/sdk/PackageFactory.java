...
public class PackageFactory {
    ...
    public static ActivityPackage buildDeeplinkSdkClickPackage(Uri uri, long j10, ActivityState activityState, AdjustConfig adjustConfig, DeviceInfo deviceInfo, GlobalParameters globalParameters, ActivityHandler.InternalState internalState) {
        String uri2;
        if (uri == null || (uri2 = uri.toString()) == null || uri2.length() == 0) {
            return null;
        }
        try {
            uri2 = URLDecoder.decode(uri2, Constants.ENCODING);
        } catch (UnsupportedEncodingException e10) {
            AdjustFactory.getLogger().error("Deeplink url decoding failed due to UnsupportedEncodingException. Message: (%s)", e10.getMessage());
        } catch (IllegalArgumentException e11) {
            AdjustFactory.getLogger().error("Deeplink url decoding failed due to IllegalArgumentException. Message: (%s)", e11.getMessage());
        } catch (Exception e12) {
            AdjustFactory.getLogger().error("Deeplink url decoding failed. Message: (%s)", e12.getMessage());
        }
    ...
    public static ActivityPackage buildReftagSdkClickPackage(String str, long j10, ActivityState activityState, AdjustConfig adjustConfig, DeviceInfo deviceInfo, GlobalParameters globalParameters, ActivityHandler.InternalState internalState) {
        String str2 = str;
        ...
        try {
            str3 = URLDecoder.decode(str, Constants.ENCODING);
        } catch (UnsupportedEncodingException e10) {
