...
public class IOContext {
    ...
    public char[] allocNameCopyBuffer(int i10) {
        ...
        char[] allocCharBuffer = this._bufferRecycler.allocCharBuffer(3, i10);
    ...
    public byte[] allocReadIOBuffer(int i10) {
        _verifyAlloc(this._readIOBuffer);
        byte[] allocByteBuffer = this._bufferRecycler.allocByteBuffer(0, i10);
        this._readIOBuffer = allocByteBuffer;
        return allocByteBuffer;
    }
    ...
    public char[] allocTokenBuffer(int i10) {
        _verifyAlloc(this._tokenCBuffer);
        char[] allocCharBuffer = this._bufferRecycler.allocCharBuffer(0, i10);
        this._tokenCBuffer = allocCharBuffer;
        return allocCharBuffer;
    }
    ...
    public byte[] allocWriteEncodingBuffer(int i10) {
        _verifyAlloc(this._writeEncodingBuffer);
        byte[] allocByteBuffer = this._bufferRecycler.allocByteBuffer(1, i10);
        this._writeEncodingBuffer = allocByteBuffer;
        return allocByteBuffer;
    }
}
